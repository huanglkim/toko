...
@section('content')
    <div class="container py-4">
        <div class="card mb-4">
            ...
            <div class="card-body">
                <form id="filter-form-bulanan" action="{{ url('neraca-saldo/print') }}" method="POST">
                    @csrf
    ...
    <div class="container py-4">
        <div class="card mb-4">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h3 class="mb-0">Laporan Neraca Tahunan </h3>
            </div>
            <div class="card-body">
                <form id="filter-form-tahunan" action="{{ url('neraca-saldo-tahunan/print') }}" method="POST">
                    @csrf
                    <div class="row g-3 align-items-end">
                        <div class="col-md-4">
                            <label for="tahun-tahunan">Tahun</label>
                            <select id="tahun-tahunan" name="tahun" class="form-control" required>
                            </select>
                        </div>
                        <div class="col-md-4">
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            document.getElementById('tipe-bulanan').addEventListener('change', function() {
                ...
                if (tipe === 'cetak') {
                    form.action = '{{ url('neraca-saldo/print') }}';
                } else if (tipe === 'excel') {
                    form.action = '{{ url('neraca-saldo/exportBulan') }}';
                }
            });

            document.getElementById('tipe-tahunan').addEventListener('change', function() {
                var form = document.getElementById('filter-form-tahunan');
                var tipe = this.value;

                if (tipe === 'cetak') {
                    form.action = '{{ url('neraca-saldo-tahunan/print') }}';
                } else if (tipe === 'excel') {
                    form.action = '{{ url('neraca-saldo-tahunan/exportTahun') }}';
                }
            });

            // Populating years dynamically
            ...
            populateYearSelect('tahun-bulanan');
            populateYearSelect('tahun-tahunan');
        });
    </script>
@endsection
